...
import numpy as np
import audio
from wavenet_util import is_mulaw, is_mulaw_quantize, mulaw, mulaw_quantize

def build_from_siwis_path(hparams, siwis_dir, mel_dir, linear_dir, wav_dir, n_jobs=8, tqdm=lambda x: x, multi_processing=True):
    """
    ...
    parties = os.listdir(chemin + "/text")
    for i in range(len(parties)):
        if i == 3: continue
        for filename in os.listdir(chemin + "/text/" + parties[i]):
            infos = filename.split("_")

            with open(chemin + "/text/" + parties[i] + "/" + filename, encoding="utf-8") as fichier:
                phrase = fichier.read()
            basename = filename[:-4]
            fichier_wav = "{}/wavs/{}/{}.wav".format(chemin, parties[i], basename)
...
def _process_utterance(mel_dir, linear_dir, wav_dir, index, wav_path, text, hparams):
    ...
    return (wav_path, audio_filename, mel_filename, linear_filename, time_steps, mel_frames, audio_time, text, len(text))
